...
clear all;
format long;
f = @(x) (cos(x))^3;  %Change here for a different function
b = (3*pi)/2;   % Upper limit of the function
a = pi/2;     % Lower limit of the function
...
for i = a:h:b       % for loop starting a to b with increment of h
    ...
    x(p) = i;      
    y(p) = (cos(i))^3;     % Change for a different function       
end
