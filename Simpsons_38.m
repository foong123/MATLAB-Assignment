...
a1 = 6;
b1 = 3;
b = pi/2;   % Upper limit of the function
a = 0;     % Lower limit of the function
k = sqrt(a1^2- b1^2)/a1;
f = @(theta) sqrt(1-(k.^2).*sin(theta).^2);  %Change here for a different function
%{
   Simpson's 3/8 Rule 
...
n = 786432;             % Number of subinterval
...
for i = a:h:b       % for loop starting a to b with increment of h
    ...
    x(p) = i;
    y(p) = sqrt(1-(k.^2).*sin(i).^2);    % Change for a different function       
end
...
answer = 4*a1*((3*h)/8) * (y(1) + 3*SumWeighti + 3*SumWeightj + 2*SumWeightk + y(L));
fprintf('Answer = %.16f\n',answer);
